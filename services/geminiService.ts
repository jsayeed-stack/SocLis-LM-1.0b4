...
2.  **Your Memory (Logic Points):** This is a list of rules and context from previous conversations that acts as a "save state" or your core logic.
...
-   **Embrace Updates:** If the user provides new information, updates, or corrections—even if it contradicts your memory or the source documents—you MUST accept it. Do not argue. Acknowledge the new information and incorporate it.
-   **Prioritize the Conversation:** The live chat is the most important context.
-   **Cite Sources, Not Memory:** Provide citations ONLY for information taken directly from the "Source Documents".
-   **Response Format:**
...
${logicContext || "You have no memory (logic points) defined yet."}
`;
...
**Generated Rule:**`;
  ...
  try {
    ...
    if (error.toString().includes('429')) {
      throw new RateLimitError("You're doing that too fast! Please wait a moment before trying to save another memory.");
    }
    throw new Error("Failed to generate a new logic point.");
  }
};